...
Starting fresh on KNJ Panel: a hosting control panel built from the ground up, not a reskin of an
existing tool. The goal is a genuinely complete feature set over time — our own provisioning
engine, our own design and identity throughout.

## The shape of it

Two layers:

- A **Controller** area where server admins and resellers manage accounts, packages, servers, and
  security tooling.
- An **account** area that every account — including the primary one — gets its own copy of, for
  managing its own sites, mail, and databases.

Multi-server from day one, rather than being locked to a single box. Both areas run on their own
dedicated ports rather than sharing the main domain — port 80/443 on the server is reserved for a
default landing page, and eventually for whatever the first hosted account puts there.
...
- Accounts, roles (Admin/Reseller/Account), and login are working — no public self-signup,
  accounts are created by an admin/reseller.
