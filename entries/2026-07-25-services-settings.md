...
built now — server-level defaults, distinct from account management.
...
SOA record fields — are now configurable instead of fixed in code, a real DNS zone template editor
rather than fixed defaults.

## Also

Service Status was missing the nameserver service from its list since it was added a couple of
phases back — fixed. A separate "restart individual services" control is a distinct feature from
settings and is intentionally left for later, rather than requesting more server access for a
smaller add-on right after yesterday's larger changes.
