# Phase 72 - A Real Feature Audit, and the Feature That Said No to Itself

Asked to go further than the roadmap doc this time: a full re-audit of the admin feature catalog,
cross-referenced against the live codebase rather than trusted from memory. Most of it held up. Two
items didn't: List Parked Domains/List Subdomains and Spamd Startup Configuration were both filed
away months ago as "Not planned, blocked on X" — and in both cases, X had already shipped, weeks
apart, without anyone circling back to un-block the note it had blocked. Two more had never been
tracked at all: Edit Database Configuration, and a proper Terminal.

Fixed the two stale ones on the spot. Then built all four real gaps, plus one more the sweep
surfaced as a genuine open question rather than a checkbox: a Security Policies overview.
...
directly rather than assume) — applied as a process priority instead, via a systemd `Nice=` drop-in
rather than anything spamd parses itself.
...
**Security Policies** — a genuine roll-up (login lockout status, password policy, host access
rules, 2FA adoption across every admin and reseller) plus one real new capability none of the
individual pages offered on their own: requiring 2FA rather than leaving it opt-in. Enforced in the
same middleware that already re-checks reseller suspension on every request — the natural place,
since it was already running there.

## The one that pushed back

Terminal was the fifth item, and the one that actually stopped and asked a question back.

A real terminal is, by definition, a root shell. This codebase made a deliberate choice from its
very first commits: the panel's own app process runs unprivileged, and every single server-touching
action goes through one allowlisted script sudoers permits with specific, validated arguments — the
entire reason a bug in the Laravel app can't become a compromised server on its own. Building a real
terminal would mean a new, broad sudoers grant undoing exactly that property, on the one feature
that exists specifically to run arbitrary commands.
...
by accident. An "everything runs as root" model is a well-known, often-criticized weakness in this
space generally — a single compromised entry point becomes instant, unconditional root, no second
step needed. This panel's split between an unprivileged app and one narrow, audited escalation path
is a deliberate, load-bearing architectural choice, not an accident of scope. The one feature where
matching a familiar shape as closely as possible and staying secure actually pulled in different
directions was resolved in favor of the property that's been true of every other feature in this
codebase since day one.
